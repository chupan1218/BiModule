## BiModule: bipartite modularity strategy for identifying gene co-regulatory modules ##
...
## directory of input datasets and output results, change to fit your workplace ##
dircode   = "C:\\Users\\panchu\\Desktop\\CBiModule\\sourcecode"
dirdata   = "C:\\Users\\panchu\\Desktop\\CBiModule\\datasets"
dirresult = "C:\\Users\\panchu\\Desktop\\CBiModule\\results"
setwd(dirresult)


## read the datasets ##
TFexpression    <- read.csv(paste(dirdata, "\\CESCTFexpression.csv",sep=""),    header = T, row.names = 1, check.names = F)
miRNAexpression <- read.csv(paste(dirdata, "\\CESCmiRNAexpression.csv",sep=""), header = T, row.names = 1, check.names = F)
mRNAexpression  <- read.csv(paste(dirdata, "\\CESCmRNAexpression.csv",sep=""),  header = T, row.names = 1, check.names = F)
mRNAregulator   <- read.csv(paste(dirdata, "\\CESCmRNAregulator.csv",sep=""),   header = T, row.names = 1, check.names = F)



V <- BiModule(mRNAregulator, TFexpression, miRNAexpression, mRNAexpression, 0.005, 0.75)


## main functions
BiModule <- function(mRNAregulator, TFexpression, miRNAexpression, mRNAexpression, lambda.tol, merge.tol) {
   #record running time
   ptm <- proc.time()
   ...
   Bicluster <- bicluster(mRNAregulator, TFexpression, miRNAexpression)
   ...
   V4 <- module(V3, mRNAregulator, TFexpression, miRNAexpression, mRNAexpression, col.id, row.id) 
...
## initialize the maxiaml bicliques as biclusters
bicluster <- function(W, TFexpression, miRNAexpression){
  ...
  temp1 <- nrow(TFexpression)
  
  temp2 <- nrow(miRNAexpression)
  
  #extract the maximal bicliques that simultaneously contain at least one TF and one miRNA
  cluster<- mclapply(cluster, function(v) {
    if(any(unlist(v$regulator) %in% 1:temp1) 
       & any(unlist(v$regulator) %in% (temp1+1):(temp1+temp2)))
      v$symbol <- 0
    v
  })
  
  cluster[sapply(cluster, function(v) (v$symbol==1))] <- NULL
...
module <- function(V, W, TFexpression, miRNAexpression, mRNAexpression, col.id, row.id){
  
  regulatorexpression <- rbind(TFexpression, miRNAexpression) 
  ...
  tempV<- mclapply(V, function(v) {
   ...
   for(i in 1:length(v$left)){
    correlation <- correlation + abs(as.numeric(cor(t(mRNAexpression[v$left[i],]), t(regulatorexpression[v$right[i],]))))
   }
